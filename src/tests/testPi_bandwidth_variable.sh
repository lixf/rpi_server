...
echo "BANDWIDTH Test: Three workers, one master"
...
do
    go run $CLIENT_GO -b=$TESTS/pure_bandwidth_25_"$i".txt > $TESTS/logs/client"$i"_pband25_10.log &
    CLIENT_PID[$i]=$!
done
...
for (( i = 0 ; i < $NUM_CLIENTS ; i++ ))
do
    echo "$(tail -n 1 logs/client"$i"_pband25_10.log | awk 'NF>1{print $NF}')"
done
